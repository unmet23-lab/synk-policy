import {createScene,stepScene} from '/practice/loom-scene.js';
const {composeEyeOnly}=await import('/practice/eye-composite.js'+new URL(import.meta.url).search);
...
export async function initBlinkDemo(region=document.querySelector('[data-blink-demo]')) {
  ...
  try {
    const manifestURL=new URL('/practice/blink-manifest.json'+new URL(import.meta.url).search,import.meta.url);
    ...
    const refresh=()=>{
      motion.textContent=running?"Pause animation":"Play animation";motion.setAttribute('aria-pressed',String(running));
      hold.textContent=held?"Show open eyes":"Show closed eyes";hold.setAttribute('aria-pressed',String(held));
      status.textContent=held?"Still image with eyes closed.":running?"The blinking animation is playing.":"Still image. Use the button to change the expression.";
    };
