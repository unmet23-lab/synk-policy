import {initOrbConversation} from '/en/orb-conversation.js?v=e98afdd971b5';
...
if(brandDeck){
 ...
 function choose(index,announce=true){
  ...
  const reveal=()=>{panels[selected].hidden=false;panels[selected].classList.add('is-revealed');if(announce)status.textContent=slides[selected].querySelector('img').alt+" selected. Use the menu below or open the brand name to continue.";};
...
if(letterFrame){
 ...
 status.hidden=false;
 let ready=false;
 const slowLoad=setTimeout(()=>{if(!ready)status.textContent="If the activity does not appear, use the link below to open it in a new window.";},12000);
...
if(location.pathname==='/en/shift/'&&location.hash==='#shift-business')location.replace('/en/shift/#shift-services');
if(location.pathname==='/en/'){
 ...
 if(brand)location.replace('/en/'+brand+'/'+(hash===brand?'':'#'+(hash==='shift-people'?'path-journey':hash==='shift-business'?'shift-services':hash)));
 else if(hash==='work')location.replace('/en/#worlds');
}
