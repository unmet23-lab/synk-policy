...
const pending=fetch('/en/knowledge.json?v=1b57969889d9').then(r=>{if(!r.ok)throw new Error("Public information could not be loaded.");return r.json();}).then(data=>{
...
async function getEngine(){if(engine)return engine;try{return await pending;}catch{const r=await fetch('/en/knowledge.json?v=1b57969889d9',{cache:'reload'});if(!r.ok)throw new Error("Public information could not be loaded. Please try again shortly.");engine=createKnowledgeEngine(await r.json());$('#answer-note').textContent=readyNote;return engine;}}
