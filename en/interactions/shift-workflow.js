...
(() => {
  document.querySelectorAll('[data-shift-workflow]').forEach((workflow) => {
    ...
    import('/en/practice/copy-request.js' + version).then(module => module.initRequestEditors(workflow));
    const observer = new IntersectionObserver(entries => {
      if (!entries.some(entry => entry.isIntersecting)) return;
      observer.disconnect();
      import('/en/practice/blink-demo.js' + version).then(module => module.initBlinkDemo(workflow.querySelector('[data-blink-demo]'))).catch(() => {
        const status = workflow.querySelector('[data-blink-status]');
        if (status) status.textContent = "The animation could not load. The original image is shown instead.";
      });
    }, {rootMargin: '120px'});
