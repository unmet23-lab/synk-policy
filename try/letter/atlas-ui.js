...
const choices={
 ...
 time:{label:'점검 분량',options:[['standard','기본 점검'],['short','짧게 연습하기'],['unlimited','충분히 연습하기']]},
...
export function createAtlasUI({storage=null,clock=()=>new Date().toISOString()}={}){
 ...
 return {
  ...
  feedbackPanel(){const options=[['helpful','적당했어요'],['too_much','확인할 내용이 많았어요'],['want_more','조금 더 연습하고 싶어요'],['not_fit','나에게 맞지 않았어요']];const next=response==='too_much'?'다음 연습에서는 점검 항목을 한 개부터 보여드립니다.':response==='want_more'?'다음 연습에서는 다른 표현을 비교하는 안내도 추가합니다. 짧게 연습하도록 설정하면 해당 설정을 우선합니다.':response==='helpful'?'다음 연습의 분량과 도움 방식을 정할 때 참고합니다.':response==='not_fit'?'다음 연습 방식을 정할 때 참고합니다. 위의 연습 설정도 직접 바꿀 수 있습니다.':'';return `<section class="atlas-feedback" aria-labelledby="atlas-feedback-title"><h2 id="atlas-feedback-title">확인할 내용과 도움은 충분했나요?</h2><p>선택하면 다음에 같은 메일을 연습할 때 점검 항목 수와 추가 안내를 조정합니다. 답하지 않아도 됩니다.</p><div class="atlas-feedback-options">${options.map(([value,label])=>`<button type="button" class="quiet" data-atlas-feedback="${value}" aria-pressed="${response===value}"${completion?'':' disabled'}>${esc(label)}</button>`).join('')}</div><p role="status">${esc(next)}</p></section>`;},
 };
}
