import {createKnowledgeEngine} from './knowledge-engine.js?v=4308844786';
import {appendPublicActions} from './public-actions.js?v=4308844786';
...
const pending=fetch('/knowledge.json?v=2026-09-15-selected-experiences-copy-e6b297b2b0').then(r=>{if(!r.ok)throw new Error('자료를 불러오지 못했어요.');return r.json();}).then(data=>{
...
async function getEngine(){if(engine)return engine;try{return await pending;}catch{const r=await fetch('/knowledge.json?v=2026-09-15-selected-experiences-copy-e6b297b2b0',{cache:'reload'});if(!r.ok)throw new Error('자료를 불러오지 못했어요. 잠시 뒤 다시 질문해 주세요.');engine=createKnowledgeEngine(await r.json());$('#answer-note').textContent=readyNote;return engine;}}
