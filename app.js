...
const pending=fetch('/knowledge.json').then(r=>{if(!r.ok)throw new Error('자료를 불러오지 못했어요.');return r.json();}).then(data=>{
...
async function getEngine(){if(engine)return engine;try{return await pending;}catch{const r=await fetch('/knowledge.json',{cache:'reload'});if(!r.ok)throw new Error('자료를 불러오지 못했어요. 잠시 뒤 다시 질문해 주세요.');engine=createKnowledgeEngine(await r.json());$('#answer-note').textContent=readyNote;return engine;}}
...
function appendAnswer(result){
  ...
  const label=element('div','message-label');const mark=element('img','answer-wordmark');mark.src='/assets/brand-synk.webp';mark.alt='SYNK';mark.width=744;mark.height=360;label.append(mark,element('span','','공개 문서 안내'));article.append(label);
